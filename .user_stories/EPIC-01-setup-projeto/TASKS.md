# EPIC-01 — Setup do projeto — Tarefas

Checklist derivada das histórias em [`EPIC-01-setup-projeto.md`](./EPIC-01-setup-projeto.md). Marcar ao terminar (critério de aceitação da história cumprido, testes incluídos).

- [ ] 01.1 — Bootstrap do repositório (falta: arranque em simulador/dispositivo e CI de 01.13)
- [ ] 01.2 — Navegação com expo-router (falta: cold start com deep link para utilizador autenticado — depende da persistência de 01.3)
- [ ] 01.3 — Estado com Zustand
- [ ] 01.4 — SQLite local como fonte de verdade
- [ ] 01.5 — Supabase e autenticação
- [ ] 01.6 — Camada de sincronização
- [ ] 01.7 — Storybook
- [ ] 01.8 — Testes
- [ ] 01.9 — Internacionalização (i18n)
- [ ] 01.10 — Estilo com NativeWind (Tailwind)
- [ ] 01.11 — Cliente HTTP, mocks e datas
- [ ] 01.12 — Firebase: Crashlytics, Push, Analytics
- [ ] 01.13 — Pipeline CI/CD
